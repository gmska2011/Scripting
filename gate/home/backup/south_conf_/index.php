...
$maskQuery = mysql_query("SELECT internet.mask, domain_user.id FROM internet, domain_user WHERE domain_user.id_object=internet.id_object and domain_user.username='".$username."' ");
$maskQueryRow = mysql_fetch_assoc($maskQuery);
var_dump($maskQueryRow);
$MASK = $maskQueryRow['mask'].'.';
$id_user = $maskQueryRow['id'];
...
if (isset($last_core)) {
	...
	echo "# UPDATE user_list SET core='".$last_core."' WHERE name_user='".$username."' \r\n";
} else { $last_core = 1; }
...
$query_shtrich = mysql_fetch_assoc(mysql_query("SELECT * FROM `shtrich` WHERE `id_user`='".$id_user."'", $dbcnx));
...
echo "IP_VES=";
$qves = mysql_query("SELECT SUBSTRING_INDEX(`ip`, '.', -1) as sip FROM ves WHERE id_object='".$id."'", $dbcnx);
$n = mysql_num_rows($qves);
for ($i = 0; $i < $n; $i++) {
	$row = mysql_fetch_assoc($qves);
	$res[] = $row;
}
if (isset($res)) {
	if (count($res) > 0) {
