...
//////// ЕСЛИ СЕГОДНЯ ПОСЛЕДНИЙ ДЕНЬ МЕСЯЦА И ВРЕМЯ БОЛЬШЕ 21
	...
	while ($trm = mysql_fetch_assoc($result_trm)){
		...
		function session_count_core ($dbcnx, $i, $my_ip_term) {
			...
			if (!$result) die(mysql_error($dbcnx));	
	...
	if (isset($_GET["pivo"])){
		...
		$query2 = sprintf("SELECT SUBSTRING_INDEX(`ip_ws`, '.', -1) as sub FROM ws WHERE id_mag='".$id."' and os_ws != 'ATOL'");
	}
	...
	$d = strtotime("-13 day",strtotime($date_time));
