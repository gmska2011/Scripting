...
#print "<table border=0 cellpadding=1 cellspacing=2>";
    ...
    $content=get("http://pitanie-tlt.ru/cgi-bin/sp/cms/cms.pl?num=107000799&date=16.09.2007") || die "Cannot get URL !";
...
$content=~/\<div id=\"account_data\"\>(\<p\>.*?\<\/p\>.*?\<\/p\>.*?\<\/p\>.*?\<\/p\>.*?)\<\/p\>/sg;
#<div id="account_data"><p>
$content=~ tr/\<p\>/ /;
$content=~ tr/\<\/p\>/ /;
#$content=~ tr/  / /;
#$content=~/\<div class=\"pampers\"\>.*?\<table\>(.*?\<\/tbody\>\<\/table\>.*?)\<\/div\>/sg;
