...
function shop_all ($dbcnx) {
    $query = "SELECT * FROM shop WHERE online=1 and gorod='TLT' ORDER BY id ASC";
...
//	    	}
            ...
            $prerun = "sshpass -f sshpass ssh -o 'StrictHostKeyChecking no' admin@".$row." 'cat /tmp/local/sbin/post-firewall'";
            exec($prerun, $output);

		    if (isset($output[0])) {

//		if ( strpos($output[0], 'EGAIS')) {
			echo "ID ".$shop_id." - ASUS DETECTED \n";
			$router="ASUS";
		    var_dump ($output);
//		 $run2 = "sshpass -f sshpass scp -o 'StrictHostKeyChecking no' ./tplink/firewall.user root@".$row.":/etc/";
//                exec($run2, $output_asus2);
