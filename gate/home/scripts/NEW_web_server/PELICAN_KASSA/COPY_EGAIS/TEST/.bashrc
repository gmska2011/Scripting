...
#./psa_fr.exe -yS -bn &        # Драйвер фискального регистратора.Букву COM порта можно посмотреть через dmesg |grep tty 
./psa_fr_uni.exe &
#./disp_usb.exe 0 &           # Табло покупателя. Подключать через USB-COM. Включить потом его в САУС под поролем администратора.
#./slim_nix.exe ttyACM1 &     # Весы тонкие Slim
./digi708.exe USB0 &         # Весы толстые digi780
#./disp_video_usb.exe 0 ./ 192.168.1.150 1001 1 1 &   # Для Миндаля. Табло покупателя и интелект. Два в одном.
./curl_nix.exe &
./wait_nix.exe &              # Open for Sberbank
...
#screen -S "dos" dosemu -k -V # Режим кассы в скрине
dosemu -k -V                  # without screen

########## Убиваем запущенные процессы при выходе из кассы ##########
