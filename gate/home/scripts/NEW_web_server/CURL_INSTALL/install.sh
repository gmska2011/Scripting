...
#####  PARAM=`mysql -N -D ip_pelican -u root -puk.rjyfn2009 -e "SELECT ws_kas.ip_kas FROM ws_kas,shop WHERE shop.online=1 and ws_kas.id_mag=shop.id and ws_kas.curl=0" `
#PARAM='192.168.209.11' 
PARAM='192.168.237.10 192.168.237.11 192.168.237.12' 
   echo $PARAM
   }
...
then                                                                                                                                                      
  echo "PING" $ip " - OK"                                                                                                                                       
    ...
    /usr/bin/sshpass -p $PASS ssh -o 'StrictHostKeyChecking no' root@$ip "apt-get update && apt-get install -y curl"
    ...
    echo "update ws_kas set curl='1' where ip_kas='$ip';" | mysql -uroot -puk.rjyfn2009 ip_pelican; 
    fi
    ### END PUPPET INSTALL ###
