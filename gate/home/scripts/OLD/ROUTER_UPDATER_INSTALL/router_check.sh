...
get_ip ()
{
    FILE=$1
    # ASUS ONLY
    PARAM=`mysql $MYSQL -e "SELECT DISTINCT(CONCAT(i.mask, '.4')) FROM internet as i,object as o WHERE o.open=1 and i.id_object=o.id and i.router NOT IN ('ASUS')"`
    echo $PARAM
}

IP_KASSA=`get_ip ip`
#IP_KASSA=$PARAM
IP_KASSA="192.168.136.4"
PASS='nhf[nty,thu01'
echo $IP_KASSA
