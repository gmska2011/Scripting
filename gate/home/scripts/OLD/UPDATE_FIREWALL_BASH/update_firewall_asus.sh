...
get_ip ()
 {
  ...
  PARAM=`mysql -N -D ip_pelican -u root -puk.rjyfn2009 -e "SELECT ws_kas.ip_kas FROM ws_kas,shop,internet WHERE shop.online=1 and internet.router='ASUS' and ws_kas.id_mag=shop.id and internet.id_mag=shop.id and shop.pcafe=1 group by shop.id" | awk -F "." '{print $1"."$2"."$3".4"}'`
