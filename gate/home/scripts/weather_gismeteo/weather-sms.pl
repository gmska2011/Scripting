...
#    
     ...
     $gorod=$1;
     $content=~/\<\/td\>\<td nowrap align\=center bgcolor\=.*?\>.*?\<span.*?\<td nowrap align\=center bgcolor\=.*?\>(.*?)\<span/;
     $day1=$1;
     $content=~/\<\/td\>\<td nowrap align\=center bgcolor\=.*?\>.*?\<span class=sml\>.*?\<span class=sml\>(.*?)\<\/span\>/;
     $day1_1=$1;
     
     $content=~/\<\/td\>\<\/tr\>\<tr\>\<td bgcolor\=.*?\>(.*?)\<\/td\>/;
     $obl=$1; 
     
     $content=~/width=40 height=40 alt=.*?width=40 height=40 alt=\"(.*?)\"/;
     $obl1=$1;
     $content=~/\"\>\<\/td\>\<\/tr\>\<tr\>\<td bgcolor\=.*?\>(.*?)\<\/td\>\<td/;
     $osadki=$1;
     
     $content=~/\<img src\=.*? width\=40 height\=20 alt\=.*?\<img src\=.*? width\=40 height\=20 alt\=\"(.*?)\"/;
     $osadki1=$1;
     
     $content=~/\&deg\;C\<\/td\>\<td align\=center bgcolor\=.*?\>.*?\<td align\=center bgcolor\=.*?\>(.*?)\<\/td\>/;
         
     $temp1=$1;
     $content=~/\,\&deg\;C.*?,\&deg\;C\<\/td\>\<td align\=center bgcolor\=.*?bgcolor\=.*?\>(.*?)\<\/td\>/;
     $temp2=$1;
#    print $1;
#    exit;
#    $content=$1;
