...
#    
     ...
     $gorod=$1;
     $content=~/\<\/td\>\<td nowrap align\=center bgcolor\=.*?\>(.*?)\<span/;
     $day1=$1;
     $content=~/\<\/td\>\<td nowrap align\=center bgcolor\=.*?\>.*?<span class=sml>(.*?)\<\/span\>/;
     $day1_1=$1;
     ...
     $content=~/\&deg\;C\<\/td\>\<td align\=center bgcolor\=.*?\>(.*?)\<\/td\>/;
     
    
     $temp1=$1;
     $content=~/\#F0F0FF\>.*?\,\&deg\;C\<\/td\>\<td align\=center bgcolor\=.*?\>(.*?)\<\/td\>/;
     $temp2=$1;
#    print $1;
#    exit;
#    $content=$1;
